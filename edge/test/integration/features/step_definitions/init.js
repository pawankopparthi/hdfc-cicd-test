...
var environment = config['hdfc-cicd-test'].env;
...
console.log('&$hdfc-cicd-test api: [' +  domains[environment] + ', ' + config['hdfc-cicd-test'].basepath + ']');

BeforeAll("BeforeFeatures", function(next) {
    if (apps[0].name) {
        console.log ("Getting Creds from DevAppsKeys.json");
        getCreds(config['hdfc-cicd-test'].app, config['hdfc-cicd-test'].product);
    } else
    {
...
Before(function(scenario, callback) {
    ...
    this.apickli = new apickli.Apickli(tokens[0],
        tokens[1]+ config['hdfc-cicd-test'].basepath,
        './test/integration/features/fixtures/');
    callback();
});
